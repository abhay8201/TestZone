...
if ($count > 0) {
    ...
    $sql = "INSERT INTO students (s_id, test_id, s_name, test_name, subject, total_questions, total_time, teacher_id, total_marks, correct_answer, not_attempt, incorrect_answer, obtained_marks, percentage,answer,status) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)";
    $stmt = $conn->prepare($sql);
    // Serialize the answers array to store it in the database
    $stmt->bind_param("iisssisiiiiiiiss", $studentID, $testID, $studentName, $testName, $subject, $total_questions, $total_time, $teacherID, $total_marks, $correct_count,$not_attempted_count, $incorrect_count, $obtainedMarks, $percentage, $answer_string,$status);
    $stmt->execute();
    $stmt->close();
}
...
<head>
    ...
    <title>TestZone - View Test Results</title>
